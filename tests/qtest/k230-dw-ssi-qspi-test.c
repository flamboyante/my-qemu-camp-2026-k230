 ...
 *
 * 测试通过真实 w25q256 验证增强事务生成结果。QEMU SSIBus 是事务级、
 * 字节级接口，因此这里验证阶段顺序与最终数据，不模拟每根 IO 线的电平。
 *
                                    ...
                                    size_t read_len)
{
    ...
    g_assert_cmpuint(read_len, >, 0);
    g_assert_cmpuint(read_len, <, K230_SSI_FIFO_DEPTH);
    ...
    /* SDK PIO 顺序：先使能并预填 instruction/address，最后选择 CS。 */
...
static void test_quad_output_read_sequence(void)
{
    ...
    /* 0x6b 是 1-1-4：instruction/address 为单线，data 为 Quad。 */
...
static void test_quad_io_mode_bits_and_dummy(void)
{
    ...
    /*
     * Winbond 0xeb 是 1-4-4，并包含一个 mode byte 和四个 dummy cycle。
     * m25p80 将 dummy cycle 建模为字节写入，因此这项测试必须真正读数，
     * 不能只检查 XIP_MD_BIT_EN/WAIT_CYCLES 是否能从寄存器读回。
     */
